...
import { generateOKUD0402009 } from '../utils/documents';
import Icon from '@/components/ui/icon';
import type { Account } from '../types/bank';

export default function Cashout() {
  const { state, addTransaction, getClientAccounts } = useBank();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (amt > foundAccount.balance) {
      toast({ title: 'Недостаточно средств', description: `На счёте ${foundAccount.balance.toLocaleString('ru-RU')} ₽`, variant: 'destructive' });
      return;
    }
  ...
  const onCountConfirm = async () => {
    ...
    const tx = await addTransaction({
      ...
      fromAccount: pendingTx.account.number,
    ...
    setLastTxId(tx.id);
    setDone(true);
    toast({ title: '✅ Наличные выданы', description: `${pendingTx.amt.toLocaleString('ru-RU')} ₽ выдано клиенту` });
    ...
    <div className="max-w-2xl animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: '#E8F5EC' }}>
          <Icon name="ArrowUpFromLine" size={22} style={{ color: '#21A038' }} />
        </div>
        <div>
          <h2 className="text-xl font-bold" style={{ color: '#1A1A2E' }}>Выдача наличных</h2>
          <p className="text-sm" style={{ color: '#6B7280' }}>ОКУД 0402009 — Расходный кассовый ордер</p>
        </div>
      </div>

      {done ? (
        <div className="sber-card p-8 text-center animate-scale-in">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: '#E8F5EC' }}>
            <Icon name="CheckCircle2" size={32} style={{ color: '#21A038' }} />
          </div>
          <h3 className="text-xl font-bold mb-2" style={{ color: '#1A1A2E' }}>Операция выполнена</h3>
          <p className="mb-2" style={{ color: '#6B7280' }}>Наличные выданы клиенту</p>
          <p className="text-sm font-bold mb-6" style={{ color: '#21A038' }}>{pendingTx?.amt.toLocaleString('ru-RU')} ₽</p>
          <p className="text-xs mb-6" style={{ color: '#9CA3AF' }}>Документ ОКУД 0402009 загружен автоматически</p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => {
              const tx = state.transactions.find(t => t.id === lastTxId);
              const client = state.clients.find(c => c.id === pendingTx?.account.clientId);
              if (tx) generateOKUD0402009({ ...tx, clientName: client?.fullName || '—', clientPassport: client?.passport, accountNumber: pendingTx?.account.number || '', cashierName: state.currentUser?.name || '' });
            }} className="sber-btn-secondary px-6 py-2.5 flex items-center gap-2">
              <Icon name="Download" size={16} />Скачать ОКУД 0402009
            </button>
        ...
        <div className="sber-card p-6">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {foundAccount && (
              <div className="p-4 rounded-xl" style={{ background: '#E8F5EC', border: '1px solid #21A038' }}>
                <div className="flex items-center gap-2 mb-3">
                  <Icon name="CheckCircle" size={18} style={{ color: '#21A038' }} />
                  <span className="font-semibold text-sm" style={{ color: '#1C3B2A' }}>Счёт найден</span>
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span style={{ color: '#6B7280' }}>Клиент: </span><span className="font-medium">{clientOfAccount?.fullName}</span></div>
                  <div><span style={{ color: '#6B7280' }}>Баланс: </span><span className="font-bold" style={{ color: '#21A038' }}>{foundAccount.balance.toLocaleString('ru-RU')} ₽</span></div>
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: '#374151' }}>Сумма выдачи (₽)</label>
              <input className="input-sber" type="number" value={amount}
                onChange={e => setAmount(e.target.value)}
                placeholder="0.00" min="1" step="0.01" />
              {foundAccount && amount && parseFloat(amount) > foundAccount.balance && (
                <p className="text-xs mt-1" style={{ color: '#DC2626' }}>Сумма превышает баланс счёта</p>
              )}
            </div>
            ...
            <button type="submit" disabled={!foundAccount || !amount} className="sber-btn-primary py-3 flex items-center justify-center gap-2">
              <Icon name="ArrowUpFromLine" size={18} />Выдать наличные
            </button>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="animate-scale-in sber-card p-8 max-w-sm mx-4 text-center">
            <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: '#FEF3C7' }}>
              <Icon name="Banknote" size={32} style={{ color: '#D97706' }} />
            </div>
            <h3 className="text-lg font-bold mb-3" style={{ color: '#1A1A2E' }}>Пересчитайте и выдайте деньги клиенту</h3>
            <p className="text-2xl font-bold mb-6" style={{ color: '#21A038' }}>{pendingTx?.amt.toLocaleString('ru-RU')} ₽</p>
            <p className="text-sm mb-6" style={{ color: '#6B7280' }}>Убедитесь, что сумма верна, и передайте наличные клиенту</p>
            <button onClick={onCountConfirm} className="sber-btn-primary w-full py-3">Деньги выданы ✓</button>
          </div>
        </div>
      )}
